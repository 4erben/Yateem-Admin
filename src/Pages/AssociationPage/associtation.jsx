            ...
            <div className="card py-2">
                    ...
                    <div className="nav-tabs-responsive bg-light border-top ">
                        <div className="nav nav-tabs nav-tabs-bottom flex-nowrap mb-0 overflow-x-auto">
                            <Link state={{itemName:"جمعيتي" , pageName:"بيانات الجمعية", subName: "البيانات الاساسية"}} to="" className={`nav-link nav-item ${activeIndex === 0? "onActive":""}`} data-toggle="tab" onClick={()=>handleClick(0)}><FiFileText size={18} /> البيانات الأساسية</Link>
                            <Link state={{itemName:"جمعيتي" , pageName:"بيانات الجمعية", subName: "بيانات التواصل"}} to="course-attendes" className={`nav-link nav-item ${activeIndex === 1? "onActive":""}`} data-toggle="tab" onClick={()=>handleClick(1)}><GoMention size={18} /> بيانات التواصل</Link>
                            <Link state={{itemName:"جمعيتي" , pageName:"بيانات الجمعية", subName: "الحسابات البنكية"}} to="course-schedule" className={`nav-link nav-item ${activeIndex === 2? "onActive":""}`} data-toggle="tab" onClick={()=>handleClick(2)}> <CiCreditCard1 size={18} /> الحسابات البنكية</Link>
                            <Link state={{itemName:"جمعيتي" , pageName:"بيانات الجمعية", subName: "اقسام التبرع"}} to="course-cats" className={`nav-link nav-item ${activeIndex === 3? "onActive":""}`} data-toggle="tab" onClick={()=>handleClick(3)}><CiHeart size={18} /> أقسام التبرعات </Link>
